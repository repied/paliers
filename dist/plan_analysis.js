...
function hideTrace(i, plan) {
    ...
    if (localStorage.getItem('showAllSatComps') === 'true') {
        ...
        displayTrace || (displayTrace = satComps.has(i));
    }
    return !displayTrace;
}
...
function plotPlan(plan) {
    ...
    const tracePN2 = {
        ...
        hovertemplate: `${t('timeLabel')}: %{x:.2f} min<br>` +
    ...
    for (let i = 0; i < N_COMPARTMENTS; i++) {
        const traceComp = {
            ...
            hovertemplate: `${t('tensionLabel')}: %{y:.2f} bar<br>`
        };
        if (hideTrace(i, plan)) {
    ...
    for (let i = 0; i < N_COMPARTMENTS; i++) {
        ...
        const traceTensionsVsPN2 = {
            ...
            hovertemplate: `${t('timeLabel')}: %{customdata[0]:.2f} min<br>` +
                `${t('depthLabel')}: %{customdata[1]:.0f} m<br>` +
                `${t('pn2ambiantLabel')}: %{x:.2f} bar<br>` +
                `${t('tensionLabel')}: %{y:.2f} bar`
        };
        if (hideTrace(i, plan)) {
            traceTensionsVsPN2.visible = 'legendonly';
        }
        ...
        if (i > 0) {
            traceMValues.showlegend = false;
        }
        if (hideTrace(i, plan)) {
            traceMValues.visible = 'legendonly';
        }
        ...
        if (i > 0) {
            traceModifiedMValues.showlegend = false;
        }
        if (hideTrace(i, plan)) {
            traceModifiedMValues.visible = 'legendonly';
        }
